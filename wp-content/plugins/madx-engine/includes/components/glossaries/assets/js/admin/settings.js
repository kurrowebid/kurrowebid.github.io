(function( $, madxEngineRestListingsConfig ) {
	...
	Vue.component( 'madx-engine-media', {
		template: '#madx_engine_media',
		data: function() {
			return {
				fileData: {},
				mediaFrame: null,
	...
	Vue.component( 'madx-engine-glossaries', {
		template: '#madx_engine_glossaries',
		data: function() {
			return {
				items: madxEngineGlossariesConfig.items,
				isBusy: false,
				editID: false,
				nonce: madxEngineGlossariesConfig._nonce,
				deleteID: false,
		...
		watch: {
			itemsOrders: function( value, oldValue ) {
				...
				this.ordersAjaxRequest = jQuery.ajax( {
					...
					dataType: 'json',
					data: {
						action: 'madx_engine_glossary_save_orders',
						orders: value,
						nonce: self.nonce,
					},
					beforeSend: function() {
		...
		methods: {
			...
			deleteItem: function( itemID, itemIndex ) {
				...
				jQuery.ajax({
					...
					dataType: 'json',
					data: {
						action: 'madx_engine_glossary_delete',
						nonce: self.nonce,
						item_id: itemID,
					},
				}).done( function( response ) {
			...
			newItem: function( event, isSample ) {
				...
				jQuery.ajax({
					...
					dataType: 'json',
					data: {
						action: 'madx_engine_glossary_save',
						nonce: self.nonce,
						item: item,
						item_id: false,
					},
	...
	Vue.component( 'madx-engine-glossary', {
		template: '#madx_engine_glossary',
		props: {
			value: {
				type: Object,
				default: function() {
		...
		data: function() {
			return {
				...
				saving: false,
				nonce: madxEngineGlossariesConfig._nonce,
				saveLabel: madxEngineGlossariesConfig.save_label,
				savingLabel: madxEngineGlossariesConfig.saving_label,
				isConverting: false,
			};
		},
		mounted: function() {
		...
		methods: {
			...
			convertSource: function() {
				...
				jQuery.ajax( {
					...
					dataType: 'json',
					data: {
						action: 'madx_engine_glossary_get_fields_from_file',
						nonce: self.nonce,
						item: self.settings,
					},
				} ).done( function( response ) {
...
})( jQuery, window.madxEngineGlossariesConfig );

( function( $ ) {
	...
	class AjaxConditions {
		...
		init() {
			for ( const condition in window.madxEnginMBAjaxConditionsHandlers ) {
				if ( 'function' === typeof window.madxEnginMBAjaxConditionsHandlers[ condition ] ) {
					window.madxEnginMBAjaxConditionsHandlers[ condition ]( jQuery );
				}
			}

			$( document ).on( 'madx-engine/meta-box/data-change', ( data ) => {
				this.updateData( data );
			} );

			$( document ).trigger( 'madx-engine/meta-box/data-change' );
		...
		updateData( data ) {
			...
			var ajaxRequest = {
				action: 'madx-engine/meta-box/update-conditions',
			...
			ajaxRequest.nonce = window.madxEnginMBAjaxConditionsSettings.nonce;
		...
		getActiveConditions() {
			return window.madxEnginMBAjaxConditionsData;
		}

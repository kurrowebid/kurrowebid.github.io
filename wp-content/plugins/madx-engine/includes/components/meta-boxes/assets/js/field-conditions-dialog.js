Vue.component( 'madx-meta-field-conditions-dialog', {
	name: 'madx-meta-field-conditions-dialog',
	template: '#madx-meta-field-conditions-dialog',
	props: {
		value: {
			type: Object,
			default: function() {
	...
	data: function() {
		return {
			...
			operatorsList: madxEngineFieldsConfig.condition_operators,
		};
	},
	created: function() {
	...
	methods: {
		...
		getGlossaryFields: function( conditionIndex, query, values ) {
			...
			return wp.apiFetch( {
				method: 'get',
				path: madxEngineFieldsConfig.api_path_search_glossary_fields + '?' + window.madxEngineTools.buildQuery( {
					query: query,
		...
		getConditionFieldSubTitle: function( conditionIndex ) {
			...
			result += '<span class="madx-engine-condition-field-value">';

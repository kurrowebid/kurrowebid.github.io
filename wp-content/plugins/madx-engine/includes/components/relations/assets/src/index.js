...
class App extends Component {
	...
	fetchItems() {
		...
		window.wp.ajax.send(
			'madx_engine_relations_get_related_items',
			{
				type: 'GET',
				data: {
	...
	render() {
		...
		return ( <div className="madx-engine-rels">
...
for ( var i = 0; i < window.madxEngineRelationsControls.length; i++ ) {

	let control = window.madxEngineRelationsControls[ i ];
	...
	if ( controlEl ) {
		render(
			<App
				...
				currentObjectID={ window.madxEngineCurrentObjectID }

(function( $, madxEngineCCTConfig ) {

	'use strict';

	window.madxEngineMB = new Vue( {
		el: '#madx_cct_form',
		template: '#madx-cct-form',
		data: {
			generalSettings: {},
			metaFields: madxEngineCCTConfig.meta_fields,
			postTypes: madxEngineCCTConfig.post_types,
			buttonLabel: madxEngineCCTConfig.edit_button_label,
			isEdit: madxEngineCCTConfig.item_id,
			helpLinks: madxEngineCCTConfig.help_links,
			showDeleteDialog: false,
			prefix: madxEngineCCTConfig.db_prefix,
			icons: madxEngineCCTConfig.icons,
			availablePositions: madxEngineCCTConfig.positions,
			defaultMenuPosition: madxEngineCCTConfig.default_position,
			restBase: madxEngineCCTConfig.rest_base,
			saving: false,
			showAPIParamsInfo: false,
			commonAPIArgs: madxEngineCCTConfig.common_api_args,
			errors: {
				name: false,
				slug: false,
			},
		...
		mounted: function() {
			...
			if ( madxEngineCCTConfig.item_id ) {

				wp.apiFetch( {
					method: 'get',
					path: madxEngineCCTConfig.api_path_get + madxEngineCCTConfig.item_id,
		...
		computed: {
			...
			fieldsForColumns: function() {
				...
				var result = [ madxEngineCCTConfig.service_fields[0] ];

				for ( var i = 0; i < this.metaFields.length; i++ ) {
					result.push( this.metaFields[ i ] );
				}

				for ( var j = 1; j < madxEngineCCTConfig.service_fields.length; j++ ) {

					if ( 'cct_single_post_id' === madxEngineCCTConfig.service_fields[ j ].name && ! this.generalSettings.has_single ) {
						continue;
					}

					result.push( madxEngineCCTConfig.service_fields[ j ] );
		...
		methods: {
			...
			ensureServiceColumns: function() {
				for ( var i = 0; i < madxEngineCCTConfig.service_fields.length; i++ ) {
					if ( ! this.generalSettings.admin_columns[ madxEngineCCTConfig.service_fields[ i ].name ] ) {
						if ( 0 === i ) {
							this.$set( this.generalSettings.admin_columns, madxEngineCCTConfig.service_fields[ i ].name, {
								enabled: true,
								prefix: '#',
								is_sortable: true,
								is_num: true,
			...
			save: function() {
				...
				var self      = this,
					hasErrors = false,
					path      = madxEngineCCTConfig.api_path_edit;

				if ( madxEngineCCTConfig.item_id ) {
					path += madxEngineCCTConfig.item_id;
				}
				...
				for ( var errKey in this.errors ) {
					...
					if ( ! self.generalSettings[ errKey ] ) {
						...
						self.$CXNotice.add( {
							message: madxEngineCCTConfig.notices[ errKey ],
							type: 'error',
							duration: 7000,
						}, 'name' );
				...
				wp.apiFetch( {
					...
					if ( response.success ) {
						if ( madxEngineCCTConfig.redirect ) {
							window.location = madxEngineCCTConfig.redirect.replace( /%id%/, response.item_id );
						} else {

							self.$CXNotice.add( {
								message: madxEngineCCTConfig.notices.success,
								type: 'success',
							} );
...
})( jQuery, window.madxEngineCCTConfig );

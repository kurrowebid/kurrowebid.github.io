var madxQueryDialog = (function () {

	"use strict";

	function madxQueryDialog( options ) {
		...
		var baseClass   = 'madx-query-dialog';
		...
		this.getOrderByOptions = function() {
			...
			let result = this.options.fields || [],
				orderByOptions = window.madxQueryDialogConfig.orderByOptions || [];
		...
		this.createControls = function() {
					...
					case 'order_repeater':
						...
						children = [
							...
							},
							{
								...
								type: 'select',
								label: 'Type',
								options: window.madxQueryDialogConfig.dataTypes,
								width: 33,
							},
						];
					...
					case 'args_repeater':
						...
						children = [
							...
							},
							{
								...
								type: 'select',
								label: 'Operator',
								options: window.madxQueryDialogConfig.operators,
								width: 25,
							},
							{
								name: 'value',
			...
			actions.className = 'madx-query-actions';

			this.$applyButton             = document.createElement( 'button' );
			this.$applyButton.className   = 'madx-query-action-done madxartwork-button madxartwork-button-default madxartwork-button-success';
			...
			this.$cancelButton.className = 'madx-query-action-cancel';
		...
		this.sendResult = function() {
			...
			let docEvent = new Event( 'madx-query-apply', { queryData: result, queryDialog: this } );
		...
		this.newRepeater = function( control, children, value, parent ) {
			...
			var Repeater = function( control, children, value, parent ) {
				...
				this.newRepeaterRow = function( name, children, value ) {
					...
					columns.className = 'madx-query-repeater-row__columns';
					...
					removeButton.className = 'madx-query-repeater-row__remove madxartwork-button madxartwork-button-default';
				...
				this.newRepeaterRowWrapper = function( name ) {
					...
					wrapper.className    = 'madx-query-repeater-row';
				...
				this.newRepeaterColumn = function( data, row ) {
					...
					column.className = 'madx-query-column madx-query-column-' + width;
				...
				$el.className = 'madx-query-repeater';
				$el.dataset.name = name;

				$rows.className = 'madx-query-repeater-rows';

				$button.className = 'madx-query-repeater-add madxartwork-button madxartwork-button-default';
				$button.type = 'button';
				...
				if ( control.label ) {
					...
					label.className = 'madx-query-repeater-label';
		...
		this.newInput = function( data ) {
			...
			controlWrap.className = 'madx-query-control';
		...
		this.newField = function( data ) {
			...
			field.className = 'madx-query-field';
			label.className = 'madx-query-label';

			if ( data.inputType ) {
				field.className += ' madx-query-field--input-type-' + data.inputType;
			}

			if ( data.layout ) {
				field.className += ' madx-query-field--layout-' + data.layout;
			}
			...
			if ( data.description ) {
				...
				desc.className   = 'madx-query-field-description';
		...
		this.errorMessage = function( notices ) {
			...
			errors.className = 'madx-query-errors';
			...
			for ( var i = 0; i < notices.length; i++ ) {
				...
				error.className = 'madx-query-error';
			...
			this.$cancelButton           = document.createElement( 'button' );
			this.$cancelButton.className = 'madx-query-errors-cancel madxartwork-button madxartwork-button-default';
			this.$cancelButton.type      = 'button';
			this.$cancelButton.innerHTML = '&times; Cancel';
	...
	return madxQueryDialog;

})();

window.madxQueryDialog = madxQueryDialog;
...
window.testDialog = new madxQueryDialog( {
	listing: 897,
	trigger: $el,
	resultTarget: $result,
...
jQuery( window ).on( 'madxartwork:init', function() {
	...
	dialogView = window.madxartwork.modules.controls.BaseData.extend( {
		...
		onReady: function() {
			...
			var self = this;
			var $trigger = self.el.querySelector( '.madx-query-trigger' );
			var $result = self.ui.input[0];

			self.madxDialog = new madxQueryDialog({
				trigger: $trigger,
		...
		onBeforeDestroy: function() {
			this.madxDialog.remove();
			this.$el.remove();
		}

	} );

	window.madxartwork.addControlView( 'madx_query_dialog', dialogView );

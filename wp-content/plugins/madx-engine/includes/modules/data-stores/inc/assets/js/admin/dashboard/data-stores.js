(function( $, dataStoresConfig ) {
	...
	Vue.component( 'madx-engine-data-stores', {
		template: '#madx_engine_data_stores',
		data: function() {
			return {
		...
		methods: {
			sanitizeStoreSlug: function( value ) {
				...
				value = window.madxEngineTools.maybeCyrToLatin( value );
			...
			saveStores: function() {
				...
				jQuery.ajax({
					...
					dataType: 'json',
					data: {
						action: 'madx_engine_data_stores_save',
						nonce: self.nonce,
...
})( jQuery, window.madxEngineDataStores );

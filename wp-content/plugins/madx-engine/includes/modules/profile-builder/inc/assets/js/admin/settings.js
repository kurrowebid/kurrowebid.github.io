(function( $, madxEngineProfileBuilder ) {

	'use strict';

	Vue.filter( 'nonAdmins', function ( roles ) {
		var result = roles.filter( function( role ) {
			return "administrator" !== role.value && "madx-engine-guest" !== role.value;
		} );

		return result;
	});

	Vue.component( 'madx-profile-macros', {
		name: 'madx-profile-macros',
		template: '#madx-profile-builder-macros',
		directives: { clickOutside: window.madxVueUIClickOutside },
		data: function() {
			return {
				isActive: false,
				macrosList: window.madxEngineProfileBuilder.user_page_title_macros,
			};
		},
		methods: {
	...
	new Vue( {
		el: '#madx_engine_profile_builder',
		template: '#madx-profile-builder',
		data: {
			settings: madxEngineProfileBuilder.settings,
			pagesList: madxEngineProfileBuilder.pages,
			notLoggedActions: madxEngineProfileBuilder.not_logged_in_actions,
			rewriteOptions: madxEngineProfileBuilder.rewrite_options,
			visibilityOptions: madxEngineProfileBuilder.visibility_options,
			userRoles: madxEngineProfileBuilder.user_roles,
			postTypes: madxEngineProfileBuilder.post_types,
			userPageImageFields: madxEngineProfileBuilder.user_page_image_fields,
			saving: false,
		},
		mounted: function() {
		...
		computed: {
			userRolesForPages: function() {
				...
				for ( var i = 0; i < this.userRoles.length; i++) {
					...
					if ( 'madx-engine-guest' !== this.userRoles[ i ].value ) {
		...
		methods: {
			...
			getPosts: function( query, ids ) {
				...
				return wp.apiFetch( {
					method: 'get',
					path: madxEngineProfileBuilder.search_api + '?' + this.buildQuery( {
						query: query,
						ids: ids,
						post_type: madxEngineProfileBuilder.search_in.join( ',' ),
					} )
				} );
			},
			...
			saveSettings: function() {
				...
				jQuery.ajax({
					...
					dataType: 'json',
					data: {
						action: 'madx_engine_save_settings',
						settings: self.settings,
						_nonce: madxEngineProfileBuilder._nonce,
					},
...
})( jQuery, window.madxEngineProfileBuilder );

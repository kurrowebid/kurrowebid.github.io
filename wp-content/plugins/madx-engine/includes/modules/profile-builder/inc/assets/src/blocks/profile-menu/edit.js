	...
	return <Fragment>
		...
		>
			...
			>
				...
				/>
				{ 'account_page' === attributes.menu_context && window.madxEngineProfileBlocksConfig.account_roles && 0 < window.madxEngineProfileBlocksConfig.account_roles.length && 
					<SelectControl
						label={ __( 'Show menu for the role' ) }
						value={ attributes.account_roles }
						options={ window.madxEngineProfileBlocksConfig.account_roles }
						onChange={ newValue => {
							setAttributes({
				...
				{ 'user_page' === attributes.menu_context && window.madxEngineProfileBlocksConfig.user_roles && 0 < window.madxEngineProfileBlocksConfig.user_roles.length && 
					<SelectControl
						label={ __( 'Show menu for the role' ) }
						value={ attributes.user_roles }
						options={ window.madxEngineProfileBlocksConfig.user_roles }
						onChange={ newValue => {
							setAttributes({
								user_roles: newValue,
							});
		...
		<Disabled>
			<ServerSideRender
				block="madx-engine/profile-menu"

(function() {
	...
	Vue.component( 'madx-engine-query-builder-geosearch-control', {
		props: [ 'value' ],
		template: `
		...
		data: function() {
			return {
				mapProvider: null,
				map: null,
				label: madxEngineGeoSearch.label,
				description: madxEngineGeoSearch.description,
				help: madxEngineGeoSearch.help,
				mapDefaults: {
		...
		mounted: function() {
			...
			this.mapProvider.markerOnClick( this.map, this.markerDefaults, ( marker ) => {
				...
				wp.apiFetch( {
					method: 'get',
					path: madxEngineGeoSearch.api + '?lat=' + position.lat + '&lng=' + position.lng,

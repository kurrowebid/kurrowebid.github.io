(function( $, madxMapFieldsSettings ) {

	'use strict';

	class madxEngineMapFields {

		constructor() {
			this.mapProvider = new window.madxEngineMapsProvider();
		...
		initFields( $scope ) {
			...
			$( '.madx-engine-map-field.cx-ui-container', $scope ).each( function() {
					...
					function( entries, observer ) {
						...
						entries.forEach( function( entry ) {
							if ( entry.isIntersecting ) {
								new madxEngineRenderMapField( $this, self.mapProvider );
	...
	class madxEngineRenderMapField {
		...
		render() {
			const fieldTemplate = wp.template( 'madx-engine-map-field' );
			const templateData = {
			...
			this.$preview      = this.$container.find( '.madx-engine-map-field__preview' );
			this.$position     = this.$container.find( '.madx-engine-map-field__position' );
			this.$search       = this.$container.find( '.madx-engine-map-field__search' );
			this.$searchInput  = this.$container.find( '.madx-engine-map-field__search input' );
			this.$searchLoader = this.$container.find( '.madx-engine-map-field__search-loader' );
			this.$searchList   = this.$container.find( '.madx-engine-map-field__search-list' );
			this.$mapFrame     = this.$container.find( '.madx-engine-map-field__frame' );
		...
		setValue( position ) {
			...
			this.setPreview( madxMapFieldsSettings.i18n.loading );
			...
			switch ( this.fieldSettings.format ) {
				...
				case 'location_address':
					...
					wp.apiFetch( {
						method: 'get',
						path: madxMapFieldsSettings.api + '?lat=' + position.lat + '&lng=' + position.lng,
						...
						if ( response.success ) {
							...
							if ( response.data ) {
								...
								self.setPreview( madxMapFieldsSettings.i18n.notFound );
							}
		...
		events() {
			this.$container.on( 'click', '.madx-engine-map-field__reset', this.resetLocation.bind( this ) );
			...
			this.$searchList.on( 'click', '.madx-engine-map-field__search-item', this.searchItemClickHandler.bind( this ) );
		...
		updateHashFieldPromise( location ) {
			...
			return wp.apiFetch( {
				method: 'get',
				path: madxMapFieldsSettings.apiHash + '?loc=' + location,
			} ).then( function( response ) {
		...
		geocodeSearch() {
			...
			wp.apiFetch( {
				method: 'get',
				path: madxMapFieldsSettings.apiLocation + '?address=' + value,
			} ).then( function( response ) {
		...
		inputSearchHandler( event ) {
			...
			wp.apiFetch( {
				method: 'get',
				path: madxMapFieldsSettings.apiAutocomplete + '?query=' + value,
				...
				if ( response.success ) {
					...
					for ( let item in response.data ) {
						...
						itemsHtml += '<li class="madx-engine-map-field__search-item"' + attrs + '>' + response.data[item].address + '</li>';
					}

				} else {
					itemsHtml = '<li class="madx-engine-map-field__search-no-results">' + response.html + '</li>';
				}
	...
	$( function () {
		new madxEngineMapFields();
	} );


})( jQuery, window.madxMapFieldsSettings );
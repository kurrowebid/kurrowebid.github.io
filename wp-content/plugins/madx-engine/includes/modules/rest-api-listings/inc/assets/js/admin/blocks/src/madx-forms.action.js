...
const { withRequestFields } = madxFBHooks;
...
function RestApiAction( props ) {
		...
		return <>
			<div className="madx-form-editor__macros-wrap">
				<TextareaControl
					className='madx-border-unset'
					label={ label( 'url' ) }
					value={ settings.url }
					help={ help( 'url' ) }
			...
			{ settings.authorization && <>
				...
				{ applyFilters(
					`madx.engine.restapi.authorization.fields.${ settings.auth_type }`,
					<></>, props,
				) }
			</> }
		</>;
...
addFilter(
	'madx.engine.restapi.authorization.fields.rapidapi',
	'madx-engine',
...
addFilter(
	'madx.engine.restapi.authorization.fields.bearer-token',
	'madx-engine',
...
addFilter(
	'madx.engine.restapi.authorization.fields.custom-header',
	'madx-engine',

(function( $, madxEngineRestListingsConfig ) {

	'use strict';

	Vue.component( 'madx-engine-rest-api-listings', {
		template: '#madx_engine_rest_api_listings',
		data: function() {
			return {
				items: madxEngineRestListingsConfig.items,
				isBusy: false,
				editID: false,
				authTypes: madxEngineRestListingsConfig.auth_types,
				nonce: madxEngineRestListingsConfig._nonce,
				deleteID: false,
			};
		},
		methods: {
			...
			deleteEndpoint: function( itemID, itemIndex ) {
				...
				jQuery.ajax({
					...
					dataType: 'json',
					data: {
						action: 'madx_engine_api_endpoint_delete',
						nonce: self.nonce,
						item_id: itemID,
					},
			...
			newEndpoint: function( event, isSample ) {
				...
				if ( isSample ) {
					item = JSON.parse( JSON.stringify( madxEngineRestListingsConfig.sample_item ) );
				}
				...
				jQuery.ajax({
					...
					dataType: 'json',
					data: {
						action: 'madx_engine_api_endpoint_save',
						nonce: self.nonce,
						item: item,
						item_id: false,
					},
	...
	Vue.component( 'madx-engine-rest-api-listing-item', {
		template: '#madx_engine_rest_api_listing_item',
		props: {
			value: {
				type: Object,
				default: function() {
		...
		data: function() {
			return {
				...
				authTypes: madxEngineRestListingsConfig.auth_types,
				nonce: madxEngineRestListingsConfig._nonce,
				saveLabel: madxEngineRestListingsConfig.save_label,
				savingLabel: madxEngineRestListingsConfig.saving_label,
...
})( jQuery, window.madxEngineRestListingsConfig );

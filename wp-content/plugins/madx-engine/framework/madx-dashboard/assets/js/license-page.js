(function () {
	...
	Vue.component( 'license-item', {
		template: '#madx-dashboard-license-item',
		...
		methods: {
			showLicenseManager: function() {
				window.madxDashboardEventBus.$emit( 'showLicenseManager' );
			},
					...
					success: function( responce, textStatus, jqXHR ) {
						...
						if ( 'success' === licenseStatus ) {
							...
							if ( 'activate' === actionType ) {
								...
								window.madxDashboardEventBus.$emit( 'addLicenseItem', {
							...
							if ( 'deactivate' === actionType ) {
								window.madxDashboardEventBus.$emit( 'removeLicenseItem', self.licenseKey );
							}
						}

						if ( 'error' === licenseStatus ) {
							if ( 'limit_exceeded' === responce.code ) {
								window.madxDashboardEventBus.$emit( 'showResponcePopup', responce );
							}
						}
					}
				} );
	...
	Vue.component( 'plugin-item-installed', {
		template: '#madx-dashboard-plugin-item-installed',
		...
		methods: {
			...
			updatePlugin: function() {
				...
				if ( this.updateActionAvaliable ) {
					...
					window.madxDashboardEventBus.$emit( 'showPopupUpdateCheck' );
				}

			},

			showPopupActivation: function() {
				window.madxDashboardEventBus.$emit( 'showPopupActivation', this.pluginData['slug'] );
			},
			...
			pluginAction: function() {
				...
				self.actionPluginRequest = jQuery.ajax( {
					...
					dataType: 'json',
					data: {
						action: 'madx_dashboard_plugin_action',
						data: {
					...
					success: function( responce, textStatus, jqXHR ) {
						...
						if ( 'success' === responce.status ) {
							...
							window.madxDashboardEventBus.$emit( 'updateUserPluginData', {
			...
			deactivateLicense: function() {
				window.madxDashboardEventBus.$emit( 'showPopupDeactivation', this.pluginData['slug'] );
			}
		}
	} );

	Vue.component( 'plugin-item-avaliable', {
		template: '#madx-dashboard-plugin-item-avaliable',
	...
	Vue.component( 'responce-info', {
		template: '#madx-dashboard-responce-info',
	...
	Vue.component( 'license-page', {
		template: '#madx-dashboard-license-page',
		...
		mounted: function() {
			...
			window.madxDashboardEventBus.$on( 'addLicenseItem', this.addLicense );
			window.madxDashboardEventBus.$on( 'removeLicenseItem', this.removeLicense );
			window.madxDashboardEventBus.$on( 'updateUserPluginData', this.updateUserPluginData );
			window.madxDashboardEventBus.$on( 'showLicenseManager', this.showLicenseManager );
			window.madxDashboardEventBus.$on( 'showPopupActivation', this.showPopupActivation );
			window.madxDashboardEventBus.$on( 'showPopupDeactivation', this.showPopupDeactivation );
			window.madxDashboardEventBus.$on( 'showPopupUpdateCheck', this.showPopupUpdateCheck );
			window.madxDashboardEventBus.$on( 'showResponcePopup', this.showResponcePopup );
		...
		methods: {
			...
			updateUserPluginData: function( data ) {
				...
				this.allmadxPlugins[ slug ] = Object.assign( {}, this.allmadxPlugins[ slug ], pluginData );

( function () {
	...
	Vue.component( 'plugin-item-registered', {
		template: '#madx-dashboard-plugin-item-registered',
		...
		computed: {
			...
			mainLinkItem: function() {
				...
				return {
					...
					'label': __( 'Go to docs', 'madx-dashboard' ),
					'target': '_blank'
				};
			},
	...
	Vue.component( 'offers-item', {
		template: '#madx-dashboard-offers-item',
	...
	Vue.component( 'extras-item', {
		template: '#madx-dashboard-extras-item',
		...
		data: function() {
			return {
				adminUrl: window.madxDashboardConfig.adminUrl || '#',
				userPlugins: window.madxDashboardConfig.userPlugins || {},
			}
		},
	...
	Vue.component( 'welcome-page', {
		...
		template: '#madx-dashboard-welcome-page',
		...
		data: function() {
			return {
				...
				allmadxPlugins: window.madxDashboardConfig.allmadxPlugins || {},
				licenseList: window.madxDashboardConfig.licenseList || [],
				avaliableBanners: window.madxDashboardConfig.avaliableBanners || [],
				offersConfig: window.madxDashboardConfig.offersConfig || [],
				extrasConfig: window.madxDashboardConfig.extrasConfig || [],
				generalConfig: window.madxDashboardConfig.generalConfig || [],
				crocoWizardData: window.madxDashboardConfig.crocoWizardData || false,
		...
		computed: {
			...
			registeredPluginList: function() {
				...
				for ( let pluginSlug in this.allmadxPlugins ) {

					if ( this.allmadxPlugins[ pluginSlug ][ 'isInstalled' ]
						&& this.allmadxPlugins[ pluginSlug ][ 'isActivated' ]
					) {
						registeredPluginList[ pluginSlug ] = this.allmadxPlugins[ pluginSlug ];
					}
				}
			...
			avaliablePluginList: function() {
				...
				for ( let pluginSlug in this.allmadxPlugins ) {
					...
					if ( ( ! this.allmadxPlugins[ pluginSlug ]['isInstalled'] )
						...
						let pluginData = this.allmadxPlugins[ pluginSlug ];
			...
			updatesMessage: function() {
				let updateMessage    = sprintf( _n( 'You can update <b>%s plugin</b>', 'You can update <b>%s plugins</b>', this.updatePluginCount, 'madx-dashboard' ), this.updatePluginCount ),
					noUpdatesMessage = __( 'All plugins updated', 'madx-dashboard' ),
					message          = ( 0 === this.updatePluginCount ) ? noUpdatesMessage : updateMessage;

				if ( ! this.isLicenseActivated ) {
					message = __( 'Activate license to update your madxPlugins', 'madx-dashboard' );
				}
			...
			avaliableToInstallMessage: function() {
				let avaliableMessage    = sprintf( _n( 'You can install <b>%s more Plugin</b> with your licence', 'You can install <b>%s more Plugins</b>  with your licence', this.avaliablePluginCount, 'madx-dashboard' ), this.avaliablePluginCount ),
					noAvaliableMessage = __( 'All available plugins are already installed', 'madx-dashboard' );
			...
			wizardSectionsTitle: function() {
				let productType = this.$root.productType,
					title = __( 'Try quickstart installation', 'madx-dashboard' );

				switch( productType ) {
					case 'lifetime':
					case 'all-inclusive':
						title = __( 'Unpack Free All-inclusive items', 'madx-dashboard' );
					break;
					case 'default':
						title = __( 'Try quickstart installation', 'madx-dashboard' );
					break;
		...
		methods: {
			getLicenseExpireMessage: function( rawDate = '' ) {
				...
				if ( expireCases.includes( rawDate ) ) {
					return __( '<b class="lifetime">Lifetime</b>', 'madx-dashboard' );
				}

				return sprintf( __( 'Licence expires <b class="expire-date">%s</b>', 'madx-dashboard' ), convertedDate );
			},
			...
			wizardActionHandle: function() {
				...
				self.actionPluginRequest = jQuery.ajax( {
					...
					dataType: 'json',
					data: {
						action: 'madx_dashboard_wizard_plugin_action',
						data: {
							plugin: 'crocoblock-wizard/crocoblock-wizard.php',
							action: action,
							nonce: window.madxDashboardConfig.nonce,
						}
					},

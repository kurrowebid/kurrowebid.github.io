...
Vue.component( 'madx-post-field-control', {
	template: '#madx-post-field-control',
	props: [ 'value', 'fields', 'metaProp', 'termsProp' ],
	data: function () {
		return {
			fieldType: '',
			fieldName: '',
			taxonomies: window.madxEngineFormSettings.taxonomies,
		};
	},
	mounted: function() {
		...
		if ( ! this.value ) {
			...
			if ( 0 <= this.fieldsProps.indexOf( this.value ) ) {
				...
				if ( this.value.includes( 'madx_tax__' ) ) {
...
Vue.component( 'madx-form-preset-editor', {
	template: '#madx-form-preset-editor',
	...
	data: function () {
		return {
			...
			taxonomies: window.madxEngineFormSettings.taxonomies,
			postTypes: window.madxEngineFormSettings.post_types,
			userFields: window.madxEngineFormSettings.user_fields,
			presetSources: window.madxEngineFormSettings.preset_sources,
			editorMounted: false,
			optionsPages: window.madxEngineFormSettings.options_list,
		};
	},
	created: function() {
...
var JEBookingFormBuilder = new Vue({
	...
	data: {
		layout: JSON.parse( JSON.stringify( madxEngineFormSettings.form_data ) ),
		result: JSON.parse( JSON.stringify( madxEngineFormSettings.form_data ) ),
		index: 1,
		showEditor: false,
		...
		currentItem: {},
		currentIndex: false,
		fieldTypes: madxEngineFormSettings.field_types,
		inputTypes:madxEngineFormSettings.input_types,
		taxonomies: madxEngineFormSettings.taxonomies,
		postTypes: madxEngineFormSettings.post_types,
		hiddenValues: madxEngineFormSettings.hidden_values,
		captcha: JSON.parse( JSON.stringify( madxEngineFormSettings.captcha ) ),
		preset: JSON.parse( JSON.stringify( madxEngineFormSettings.preset ) ),
		mimes: madxEngineFormSettings.all_mimes,
		userFields: madxEngineFormSettings.user_fields,
		generatorsList: madxEngineFormSettings.generators_list,
		listingItems: madxEngineFormSettings.listing_items,
		editDynamicSettings: false,
		prevDefault: null,
		fieldOptionsSources: madxEngineFormSettings.options_sources,
	},
	mounted: function () {
	...
	methods: {
		...
		itemInstance: function( item ) {
			...
			var instance = madxEngineFormSettings.labels.field;

			if ( item.settings.is_message ) {
				instance = madxEngineFormSettings.labels.message;
			}

			if ( item.settings.is_submit ) {
				instance = madxEngineFormSettings.labels.submit;
			}
		...
		addField: function( isSubmit, isMessage, isPageBreak ) {
			var maxY            = 0,
				currY           = 0,
				newItem         = {},
				defaultSettings = JSON.parse( JSON.stringify( madxEngineFormSettings.default_settings ) );
		...
		removeField: function( item, index ) {
			...
			if ( ! confirm( madxEngineFormSettings.confirm_message ) ) {
				return;
			}
		...
		getCaptchaVal: function ( name ) {
			return this.captcha.use_global
				? madxEngineFormSettings.global_tabs.captcha[ name ]
				: this.captcha[ name ]
		}
	}
});
...
var JEBookingFormNotifications = new Vue({
	...
	data: {
		items: JSON.parse( JSON.stringify( madxEngineFormSettings.notifications_data ) ),
		index: 1,
		showEditor: false,
		currentItem: {},
		currentIndex: false,
		availableTypes: madxEngineFormSettings.notification_types,
		postTypes: madxEngineFormSettings.post_types,
		postStatuses: madxEngineFormSettings.post_statuses,
		optionsPages: madxEngineFormSettings.options_pages,
		userFields: madxEngineFormSettings.user_fields,
		postProps: madxEngineFormSettings.post_props,
		userProps: madxEngineFormSettings.user_props,
		allPages: madxEngineFormSettings.pages,
		redirectNotice: madxEngineFormSettings.labels.redirect_notice,
		activecampFields: madxEngineFormSettings.activecamp_fields,
		userRoles: madxEngineFormSettings.user_roles,
		requestProcessing: false,
		globalTabs: madxEngineFormSettings.global_tabs
	},
	components: {
	...
	methods: {
		...
		getMailChimpData: function( event, isValidate ) {
			...
			jQuery.ajax( {
				...
				type: 'POST',
				data: {
					'action': 'madx_engine_forms_get_mailchimp_data',
					'api_key': api_key
				},
		...
		getGetResponseData: function( event, isValidate ) {
			...
			jQuery.ajax( {
				...
				type: 'POST',
				data: {
					'action': 'madx_engine_forms_getresponse_data',
					'api_key': api_key
				},
				success: function( response ) {
					if ( response.success ) {
		...
		globalTab: function ( type, key = '' ) {
			let tabName = madxEngineFormSettings.pairs_notifications_tabs[ type ] || false;
			if ( ! tabName ) {
				tabName = type;
			}
			const globalTab = tabName ? madxEngineFormSettings.global_tabs[ tabName ] : {};
...
function JEBookingFormSetMessages() {
	var $messages = jQuery('#messages-settings .messages-list'),
		messages_data = madxEngineFormSettings.messages;

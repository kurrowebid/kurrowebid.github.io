...
export function getProviderFilters(provider, queryId = 'default') {
	return getNesting(madxSmartFilters, 'filterGroups', provider + '/' + queryId, 'filters') || [];
}

export function getCallbackArgs( arg ) {

	if ( ! window.madxEngineListingData.filterCallbacksArgs ) {
		return [];
	}

	const result = [];

	for ( var i = 0; i < window.madxEngineListingData.filterCallbacksArgs.length; i++ ) {

		var control = window.madxEngineListingData.filterCallbacksArgs[ i ];

...
addFilter(
	'madxEngine.dynamic.dataSourceControls',
	'madx-engine/dynamic-functions-controls',
	DynamicFunctions,
	900
);
...
const DynamicBlockControls = function( props ) {
	...
	if ( ! props.attributes.madxEngineDynamicData ) {
		return ( null );
	}

	const supports   = wp.data.select( 'core/blocks' ).getBlockSupport( props.name, 'madxEngineDynamicData' );
	const attributes = props.attributes.madxEngineDynamicData;
	...
	const resetValue = function( attr, dynamicAttributes, setAttributes, supports ) {
		...
		setAttributes( {
			madxEngineDynamicData: _.assign( {}, dynamicAttributes, { [ attr ]: { data_source: '' } } ),
		} );
		...
		for ( var i = 0; i < supports.length; i++ ) {
			...
			if ( supports[ i ].attr === attr && supports[ i ].rewrite ) {
				...
				if ( 'string' === typeof oldVal ) {
					...
					let defaults = wp.blocks.getBlockAttributes( 'madx-engine/dynamic-field', '' ) || {};
	...
	const setValue = function( newValue, key, attr, object, setAttributes, supports ) {
		...
		setAttributes( {
			madxEngineDynamicData: _.assign( {}, object, { [ attr ]: newSettings } ),
		} );
	...
	return (
		<BlockControls>
			<ToolbarGroup>
				<Dropdown
					className="madx-engine-dynamic-dropdown"
					contentClassName="madx-engine-dynamic-dropdown--content"
					...
					renderContent={ () => (
						...
						{ supports.map( ( itemData, index ) => {
							return (
								<Dropdown
									className="madx-engine-dynamic-source"
									contentClassName="madx-engine-dynamic-source--inner-content"
									position="bottom right"
									...
									renderContent={ () => (
										<Fragment>
											...
											<Button
												...
												variant="tertiary"
												isDestructive
												className="madx-engine-dynamic-source--reset"
												onClick={ () => {
													resetValue(
														itemData.attr,
														attributes,

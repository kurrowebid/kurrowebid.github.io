...
import madxEngineRepeater from "components/repeater-control.js";
...
const blockAttributes = window.madxEngineListingData.atts.listingGrid;

registerBlockType( 'madx-engine/listing-grid', {
	title: __( 'Listing Grid' ),
	icon: GIcon,
	category: 'madx-engine',
	attributes: blockAttributes,
	className: 'madx-listing-grid',
	edit: class extends wp.element.Component {
		...
		render() {
			...
			const listingOptions = window.madxEngineListingData.listingOptions;
			const hideOptions    = window.madxEngineListingData.hideOptions;
			const taxonomies     = [];
			const conditions     = {};
			...
			if ( window.madxEngineListingData.taxonomies.length ) {
				for ( var i = 0; i < window.madxEngineListingData.taxonomies.length; i++ ) {
					for ( var j = 0; j < window.madxEngineListingData.taxonomies[i].values.length; j++ ) {
						taxonomies.push( window.madxEngineListingData.taxonomies[i].values[j] );
					}
				}
			}
			...
			const userRoles = window.madxEngineListingData.userRoles;
				...
				props.isSelected && (
					...
					>
						<PanelBody title={ __( 'General' ) }>
							...
							{ ! window.madxEngineListingData.legacy.is_disabled && <Fragment>
								<ToggleControl
								...
								{ attributes.use_custom_post_types && <SelectControl
									...
									options={ window.madxEngineListingData.postTypes }
									onChange={ newValue => {
							...
							{ window.madxEngineListingData.injections.enabled &&
								<div>
									<ToggleControl
												...
												( item ) =>
													<div>
														...
														{ 1 == item.static_item && <SelectControl
															...
															options={ window.madxEngineListingData.allowedContextList }
															onChange={ newValue => {
						 	...
						 	<div dangerouslySetInnerHTML={ { __html: window.madxEngineListingData.legacy.message } }></div>
						</PanelBody>
						<PanelBody
						...
						>
							...
							{ attributes.custom_query && <SelectControl
								...
								options={ window.madxEngineListingData.queriesList }
								onChange={ newValue => {
									props.setAttributes( { custom_query_id: newValue } );
								}}
							/> }
						</PanelBody>
						{ ! window.madxEngineListingData.legacy.is_disabled && <PanelBody
						...
						>
										...
										<div>
											...
											{ 'posts_params' === item.type &&
												<div>
													...
													/>
													{ window.madxEngineListingData.dataStores && window.madxEngineListingData.dataStores.length &&
														<SelectControl
															label={ __( 'Get posts from store' ) }
															value={ item.posts_from_data_store }
															options={ madxEngineListingData.dataStores }
															onChange={ newValue => {
							...
							<p>
								<ExternalLink href="https://crocoblock.com/knowledge-base/articles/madxengine-macros-guide/">{ __( 'Macros Guide' ) }</ExternalLink>
							</p>
						</PanelBody> }
						{ ! window.madxEngineListingData.legacy.is_disabled && <PanelBody
						...
						>
							...
							/>
							<div className="madx-engine-heading">Meta Query</div>
							<madxEngineRepeater
						...
						{ ! window.madxEngineListingData.legacy.is_disabled && window.madxEngineListingData.customPanles.listingGrid.length && <React.Fragment>
							{ window.madxEngineListingData.customPanles.listingGrid.map( ( Panel ) => {
								return <Panel

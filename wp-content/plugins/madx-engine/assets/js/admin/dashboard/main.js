(function( $, dashboardConfig ) {
	...
	Vue.component( 'madx-video-embed', {
		props: [ 'embed' ],
		template: `
			<div class="madx-engine-module-video"><iframe width="500" height="281" :src="embed" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>
		`
	});

	window.madxEngineDashboard = new Vue( {
		el: '#madx_engine_dashboard',
		data: {
		...
		methods: {
			...
			processUpdate: function( file ) {
				...
				jQuery.ajax({
					...
					dataType: 'json',
					data: {
						action: 'madx_engine_update_module',
						file: file,
			...
			uninstallExternalModule: function( module ) {
				...
				jQuery.ajax({
					...
					dataType: 'json',
					data: {
						action: 'madx_engine_uninstall_module',
						module: module,
						_nonce: dashboardConfig._nonce,
					},
			...
			installExternalModule: function( module ) {
				...
				jQuery.ajax({
					...
					dataType: 'json',
					data: {
						action: 'madx_engine_install_module',
						module: module,
						_nonce: dashboardConfig._nonce,
					},
				}).done( function( response ) {
			...
			saveModules: function() {
				...
				jQuery.ajax({
					...
					dataType: 'json',
					data: {
						action: 'madx_engine_save_modules',
			...
			getForms: function( query ) {
				return wp.apiFetch( {
					method: 'get',
					path: dashboardConfig.api_path_search + '?query=' + query + '&post_type=madx-engine-booking',
				} );
...
})( jQuery, window.madxEngineDashboardConfig );

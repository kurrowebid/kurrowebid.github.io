const getLocalized = ( actionType, objectKey = '' ) => {
	const preparedAction = window.madxFormActionTypes.find( action => actionType === action.id );
	if ( ! preparedAction ) {
		return false;
	}
...
const getActionLabel = ( type ) => {
	const action = window.madxFormActionTypes.find( el => el.id === type );
...
const isInDefaultFlow = actionType => {
	const flow = 'madx_Form_Builder\\Actions\\Executors\\Action_Default_Executor';

	return flow === getLocalized( actionType, '__flow' );
};

window.madxFBLocalizeHelper = {
	getActionLabel,

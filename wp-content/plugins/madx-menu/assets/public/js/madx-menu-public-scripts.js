	...
	class madxMegaMenuClass {
		...
		constructor( instance, options ) {
			this.defaultSettings = {
				menuId: '0',
				menuUniqId: window.madxMenu.getUniqId(),
				layout: 'horizontal',
		...
		createInstance() {
			...
			this.$document.trigger( 'madxMegaMenuInited' );
		}

		initEvents() {
			let debounceTimer,
				eventHandler = 'madxMegaMenu',
				...
				case 'click':
					...
					this.$instance.on( `click.${ eventHandler }`, $itemTargetSelector, ( event ) => {
						...
						let $currentTarget  = $( event.currentTarget ),
						    ...
						    $menuItemLink   = $( '.madx-mega-menu-item__link', $menuItem ).first(),
		...
		initWatcher( delay = 10 ) {
			$( window ).off( `resize.madxMegaMenu${ this.settings['menuUniqId'] }` ).on( `resize.madxMegaMenu${ this.settings['menuUniqId'] } orientationchange.madxMegaMenu${ this.settings['menuUniqId'] }`, this.debounce( delay, this.watchTick.bind( this ) ) );
		...
		maybeTemplateLoad( templateId, templateContent, $templateContainer ) {
			...
			let getMegaContentUrl = 'default' === templateContent ? window.madxMenuPublicSettings.getBlocksTemplateApiUrl : window.madxMenuPublicSettings.getmadxartworkTemplateApiUrl;
			...
			$.ajax( {
				...
				success: function( responce, textStatus, jqXHR ) {
					...
					madxMenu.megaContentRender( $( '.madx-mega-menu-mega-container__inner', $templateContainer ), templateContent );
				}
			} );
		}
	}
...
( function( $ ) {
	...
	window.madxMenu = {
		...
		init: function() {
			...
			window.addEventListener( 'madxMenu/editor/templateRenderer/renderSuccess', ( event ) => {
				this.initLocationMenuRender();
				this.mobileVueComponents();
				this.initMobileRender();
			}, false );

			$( window ).on( 'madx-menu/ajax/frontend-init', ( event, payload ) => {
				madxMenu.maybemadxartworkFrontendInit( payload.$container );
			} );

			$( window ).on( 'madx-menu/ajax/frontend-init/after', ( event, payload ) => {
				this.initLocationMenuRender();
				this.mobileVueComponents();
				this.initMobileRender();
			} );
		...
		initLocationMenuRender: function() {
			...
			let $megaMenuList = $( '.madx-mega-menu--location-wp-nav' );
			...
			$megaMenuList.each( function() {
				...
				$this.madxMegaMenu( {
					...
					classes: {
						instance: 'madx-mega-menu',
						menuContainer: 'madx-mega-menu-container',
						menuList: 'madx-mega-menu-list',
						menuItem: 'madx-mega-menu-item',
						menuItemLink: 'madx-mega-menu-item__link',
						subMenuContainer: 'madx-mega-menu-sub-menu',
						subMenuList: 'madx-mega-menu-sub-menu__list',
						megaContainer: 'madx-mega-menu-mega-container',
					}
				} );
			} );
		},
		...
		initMobileRender: function() {
			...
			mobileMenuList.each( function() {
				...
				madxMenu.createMobileRenderInstance( menuInstanceId, menuId, menuOptions );
			} );
		},

		createMobileRenderInstance: function( menuInstanceId, menuId, menuOptions ) {

			if ( madxMenu.initedMobileRenderInstance.includes( menuInstanceId ) ) {
				return;
			}

			madxMenu.initedMobileRenderInstance.push( menuInstanceId );
			...
			Vue.component( 'mobile-menu-item', {
				...
				computed: {
					...
					getTemplateUrl: function() {
						return 'default' === this.itemDataObject.megaContentType ? window.madxMenuPublicSettings.getBlocksTemplateApiUrl : window.madxMenuPublicSettings.getmadxartworkTemplateApiUrl;
					},
				...
				methods: {
					...
					showTemplateContent: function() {
						...
						this.$nextTick( function() {
							...
							madxMenu.megaContentRender( $templateContainer );
						} );
					},
					...
					initTemplatesContent: function() {
						...
						this.$nextTick( function() {
							...
							if ( vueInstance.headerContent ) {
								...
								madxMenu.megaContentRender( $headerContainer );
							}

							if ( vueInstance.beforeContent ) {
								let $beforeContainer = $( vueInstance.$refs['before-template-content'] );

								madxMenu.megaContentRender( $beforeContainer );
							}

							if ( vueInstance.afterContent ) {
								let $afterContainer = $( vueInstance.$refs['after-template-content'] );

								madxMenu.megaContentRender( $afterContainer );
							}
						} );
					},
		...
		megaContentRender: function( $templateContainer, templateContent ) {
			...
			Promise.all( madxMenu.addedAssetsPromises ).then( function( value ) {
				...
				// Frontend init
				$( window ).trigger( 'madx-menu/ajax/frontend-init', {
					$container: $templateContainer,
					content: content,
				} );

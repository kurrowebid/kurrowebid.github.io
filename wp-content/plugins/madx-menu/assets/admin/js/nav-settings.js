(function( $, navSettingsConfig ) {
	...
	window.madxMenuNavSettings = {
		...
		initEvents: function() {
			$( document ).on( 'click.madxMenuAdmin', '.madx-menu-item-settings__trigger', this.openItemSettingPopup );
		},
		...
		initTriggers: function() {
			...
			$( '#menu-to-edit .menu-item' ).each( function() {
				let $this           = $( this ),
					depth           = madxMenuNavSettings.getItemDepth( $this ),
					id              = madxMenuNavSettings.getItemId( $this ),
					infoTemplate    = '';

				$this.addClass( 'madx-menu-item' );

				if ( itemsSettings.hasOwnProperty( id ) && itemsSettings[ id ].hasOwnProperty( 'enabled' ) ) {
					if ( 'true' === itemsSettings[ id ][ 'enabled' ] ) {
						infoTemplate = `<span class="madx-menu-item-settings__info-label mega-enabled">${ navSettingsConfig.labels.itemMegaEnableLabel }</span>`;
					}
				}

				let triggerTemplate = `<span class="madx-menu-item-settings__trigger" data-item-id="${ id }" data-item-depth="${ depth }">${ navSettingsConfig.labels.itemTriggerLabel }</span>`;

				//$this.find( '.item-title' ).append( `<span class="madx-menu-item-settings"><span class="madx-menu-item-settings__info">${ infoTemplate }</span>${ triggerTemplate }</span>` );
				$this.append( `<span class="madx-menu-item-settings"><span class="madx-menu-item-settings__info">${ infoTemplate }</span>${ triggerTemplate }</span>` );
			});
		...
		initNavSettingsInstance: function() {
			...
			if ( ! $( '#madx-menu-settings-fields' )[0] ) {
				return;
			}

			this.navSettingsInstance = new Vue( {
				el: '#madx-menu-settings-fields',
				...
				watch: {
					...
					/*controlData: {
						handler( options ) {
							...
							let $infoHolder =  $( `#menu-item-${ this.itemId } .madx-menu-item-settings__info` );

							$( '.mega-enabled', $infoHolder ).remove();

							if ( 'true' === options.enabled.value ) {
								$( `#menu-item-${ this.itemId } .madx-menu-item-settings__info` ).html( `<span class="madx-menu-item-settings__info-label mega-enabled">${ navSettingsConfig.labels.itemMegaEnableLabel }</span>` );
							} else {
				...
				methods: {
					...
					getItemData: function() {
						...
						this.ajaxAction = $.ajax( {
							...
							dataType: 'json',
							data: {
								action: 'madx_get_nav_item_settings',
								data: {
									itemId: self.itemId,
					...
					saveItemSettings: function() {
						...
						this.ajaxAction = $.ajax( {
							...
							dataType: 'json',
							data: {
								action: 'madx_save_nav_item_settings',
								data: {
									itemId: self.itemId,
									itemDepth: self.itemDepth,
							...
							success: function( responce, textStatus, jqXHR ) {
								...
								let $infoHolder =  $( `#menu-item-${ self.itemId } .madx-menu-item-settings__info` );

								$( '.mega-enabled', $infoHolder ).remove();

								if ( 'true' === self.controlData.enabled.value ) {
									$( `#menu-item-${ self.itemId } .madx-menu-item-settings__info` ).html( `<span class="madx-menu-item-settings__info-label mega-enabled">${ navSettingsConfig.labels.itemMegaEnableLabel }</span>` );
								}
		...
		openItemSettingPopup: function() {
			...
			madxMenuNavSettings.navItemsSettingsInstance.$data.itemSettingItem = true;
			madxMenuNavSettings.navItemsSettingsInstance.$data.itemId = itemId;
			madxMenuNavSettings.navItemsSettingsInstance.$data.itemDepth = itemDepth;
		},
	...
	window.madxMenuNavSettings.init();

})( jQuery, window.madxMenuNavSettingsConfig );

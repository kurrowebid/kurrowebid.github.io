( function () {
	...
	Vue.component( 'plugin-settings-toggle', {
		...
		template: '#madx-dashboard-plugin-settings-toggle',
		...
		mounted: function() {
			let activeSettingsCategory = madxDasboard.getLocalStorageData( 'activeSettingsCategory', false );
		...
		methods: {
			...
			navigateHandler: function( link, moduleSlug, category ) {
				...
				window.madxDashboardEventBus.$emit( 'settingsPage/navigateHandler', {
					'link': link,
	...
	Vue.component( 'settings-page', {
		...
		template: '#madx-dashboard-settings-page',
		...
		data: function() {
			return {
				categoryList: window.madxDashboardConfig.categoryList || [],
				proccesingState: false
			};
		},

		mounted: function() {
			window.madxDashboardEventBus.$on( 'settingsPage/navigateHandler', this.navigateHandler );
		},
		...
		methods: {
			navigateHandler: function( payload ) {
				...
				if ( moduleSlug !== this.$root.subPageModule ) {
					...
					madxDasboard.setLocalStorageData( 'activeSettingsCategory', category );

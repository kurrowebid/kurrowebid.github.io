...
class madxDasboardClass {
	 ...
	 */
	constructor() {
		//this.madxDasboardPageInstance = false;
	...
	initVueComponents() {
		...
		Vue.component( 'madx-dashboard-before-content', {
			template: '#madx-dashboard-before-content',
		...
		Vue.component( 'madx-dashboard-header', {
			template: '#madx-dashboard-header',
		...
		Vue.component( 'madx-dashboard-before-component', {
			template: '#madx-dashboard-before-component',
		...
		Vue.component( 'madx-dashboard-inner-component', {
			template: '#madx-dashboard-inner-component',
		...
		Vue.component( 'madx-dashboard-after-component', {
			template: '#madx-dashboard-after-component',
		...
		Vue.component( 'madx-dashboard-before-sidebar', {
			template: '#madx-dashboard-before-sidebar',
		...
		Vue.component( 'madx-dashboard-sidebar', {
			template: '#madx-dashboard-sidebar',
			...
			data: function() {
				return {
					themeInfo: window.madxDashboardConfig.themeInfo || false,
		...
		Vue.component( 'madx-dashboard-after-sidebar', {
			template: '#madx-dashboard-after-sidebar',
		...
		Vue.component( 'madx-dashboard-banner', {
			template: '#madx-dashboard-banner',
		...
		Vue.component( 'madx-dashboard-alert-list', {
			template: '#madx-dashboard-alert-list',
		...
		Vue.component( 'madx-dashboard-alert-item', {
			template: '#madx-dashboard-alert-item',
			...
			methods: {
				closeAlert: function() {
					let localStorageNotices = madxDasboard.getLocalStorageData( 'notices', {} );

					localStorageNotices[ this.config.id ] = {
						closed: true
					}

					madxDasboard.setLocalStorageData( 'notices', localStorageNotices );
		...
		Vue.component( 'plugin-item-more', {
			template: '#madx-dashboard-plugin-item-more',
	...
	initDashboardPageInstance() {
		...
		return new Vue( {
			...
			data: {
				pageModule: window.madxDashboardConfig.pageModule || false,
				subPageModule: window.madxDashboardConfig.subPageModule || false,
				pageConfig: window.madxDashboardConfig.pageModuleConfig,
				licenseList: window.madxDashboardConfig.licenseList || [],
				themeInfo: window.madxDashboardConfig.themeInfo || false,
				helpCenterConfig: window.madxDashboardConfig.helpCenterConfig,
				avaliableBanners: window.madxDashboardConfig.avaliableBanners,
				noticeList: window.madxDashboardConfig.noticeList || [],

				serviceAction: '',
				serviceActionsVisible: false,
				serviceActionProcessed: false,
				ajaxServiceAction: null,
				serviceActionOptions: window.madxDashboardConfig.serviceActionOptions || [],
			},

			mounted: function() {

				this.$refs.madxDashboardPage.classList.add( 'inited' );
				...
				if ( jQuery( '#toplevel_page_madx-dashboard' )[0] ) {
					let wpSubmenuItems = jQuery( '#toplevel_page_madx-dashboard .wp-submenu > li' ),
			...
			methods: {
				executeServiceAction: function() {
					...
					self.ajaxServiceAction = jQuery.ajax( {
						type: 'POST',
						url: window.madxDashboardConfig.ajaxUrl,
						dataType: 'json',
						data: {
							action: 'madx_dashboard_debug_action',
							data: {
	...
	getLocalStorageData( _key = false, _default = false ) {
		try {
			let madxDasboardData = JSON.parse( window.localStorage.getItem( 'madxDasboardData' ) );

			if ( _key ) {

				if ( madxDasboardData.hasOwnProperty( _key ) ) {
					return madxDasboardData[ _key ];
				} else {
					return _default;
				}
			}

			return madxDasboardData;
		} catch ( e ) {
			return _default;
		}
	}
	...
	setLocalStorageData( key, data ) {
		let madxDasboardData = this.getLocalStorageData() || {};

		madxDasboardData[ key ] = data;

		window.localStorage.setItem( 'madxDasboardData', JSON.stringify( madxDasboardData ) );
	}
}

(function( $ ) {
    ...
    var VisualTopologyAnimation = {
        initTopology: function () {
            madxartworkFrontend.hooks.addAction('frontend/element_ready/section', VisualTopologyAnimation.initTopologyWidget);
        },
    ...
    function topologyAnimation(target,settings,sectionId) {
        ...
        if (checkElement.length >= 0) {
            ...
            var topologyZindex = document.querySelector('.marvy-custom-topology-animation-section-'+sectionId+' .madxartwork-container');
            topologyZindex.style.zIndex = '99';

            // Set min height
            var topologyMinHeight = document.querySelector(".madxartwork-element-"+sectionId);
            topologyMinHeight.closest('.madxartwork-top-section').style.minHeight = "100px";
    ...
    $( window ).on('madxartwork/frontend/init', VisualTopologyAnimation.initTopology);
})( jQuery );

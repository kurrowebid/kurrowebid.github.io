(function( $ ) {
    ...
    var MarvyColorDropAnimation = {
        initColorDrop: function () {
            madxartworkFrontend.hooks.addAction('frontend/element_ready/section', MarvyColorDropAnimation.initColorDropWidget);
        },
        initColorDropWidget: function ($scope) {
            let sectionId = $scope.data('id');
            let target = '.madxartwork-element-'+ sectionId;
            let settings = {};
            if (window.isEditMode || window.madxartworkFrontend.isEditMode()) {
                ...
                colorDropEditorElements = window.madxartwork.elements;
    ...
    $( window ).on('madxartwork/frontend/init', MarvyColorDropAnimation.initColorDrop);
    ...
    function colorDropAnimation(target,settings,sectionId,sectionKey) {
        ...
        if (checkElement.length <= 0 ) {
            ...
            let zIndex = document.querySelector('.marvy-custom-color-drop-section-'+sectionId+' .madxartwork-container');
            zIndex.style.zIndex = '99';

            let colorDropMinHeight = document.querySelector(".madxartwork-element-"+sectionId);
            colorDropMinHeight.closest('.madxartwork-top-section').style.minHeight = "400px";
